...
export function WelcomeView(): React.JSX.Element {
  ...
  return <section className="welcome-view"><div className="welcome-hero"><div className="welcome-logo"><CircuitBoard size={32}/></div><p className="eyebrow">FPGA Studio 3.0</p><h1>See the whole design. Prove it on silicon.</h1><p>A local hardware-intelligence workspace that connects RTL, implemented paths, physical capture, and evidence-backed optimization.</p><div className="welcome-actions"><button className="primary-button" onClick={openProjectWizard}><Plus size={16}/> New FPGA project</button><button className="secondary-button" onClick={() => setView("traceability")}><Activity size={16}/> Trace the design</button></div></div><div className="welcome-columns"><article><h2>Start</h2>{starts.map((item) => <button className="start-link" key={item.label} onClick={item.run}><ChevronRight size={14}/>{item.label}</button>)}</article><article><h2>Learn</h2>{lessons.map(({i:Icon,t,s,run}) => <button className="lesson-card" key={t} onClick={run}><Icon size={18}/><span><strong>{t}</strong><small>{s}</small></span><ArrowRight size={14}/></button>)}</article><article><h2>What’s new</h2><div className="release-card"><span className="release-version">3.0 RELEASE</span><h3>Hardware intelligence, end to end</h3><ul><li>RTL-to-physical design traceability</li><li>Integrated on-chip logic analyzer</li><li>Evidence-aware design health</li><li>Build snapshots and safe experiments</li></ul><button className="text-button" onClick={() => window.dispatchEvent(new Event("fpga-studio:release-notes"))}><BookOpen size={14}/> Read release notes</button></div></article></div><div className="welcome-tip"><Sparkles size={16}/><span><strong>Smart action center:</strong> press <kbd>Ctrl K</kbd> anywhere for the recommended next step, tools, views, and theme controls.</span></div></section>;
}
