...
function Invoke-Detect {
    ...
    if ($diagnosis -match 'unable to open ftdi device|usb_open') {
        throw 'Windows can see the JTAG adapter but openFPGALoader cannot claim Interface 0. Close other programmer tools, reconnect the board, and retry. On a Primer Dock only Interface 0 uses WinUSB; leave Interface 1 as the UART driver.'
    }
